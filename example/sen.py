...
class BuildOutput(Options):
    def __init__(self, name, **kwargs):
        Options.__init__(self, kwargs)
        self.name = name # Output file name
        self.builddir = kwargs.get('builddir', None) # the specific output directory
        self.objdir   = kwargs.get('objdir', None)   # the specific object directory
        self.target   = kwargs.get('target', None)
        self.files    = None
        self.type     = kwargs.get('type', 'executable')
...
class Project(Options):
    ...
    def add_executable(self, exe):
        if not isinstance(exe, BuildOutput):
            raise ValueError("argument passed must be a 'BuildOutput'")
    ...
    def dump(self, stream, generator=True):
        ...
        for exe in self.executables:
            ...
            # Haphazard solution to building static libraries.
            ruleAssociations = { "executable" : "link", "static_lib" : "ar" }
            buildrule = ruleAssociations[exe.type]

            # final link step
            builddir = exe.builddir if exe.builddir else self.builddir
            self.ninja.build(os.path.join(builddir, exe.name), buildrule, inputs=object_files, variables=exe_variables)
            if exe.target != None:
